...
target_simu = input('Enter the name of the simulation you want as a target (e.g ULB_fETISh-KoriBU2): ')
target_exp = input('Enter the experiment (e.g expAE06): ')
target_year = input('Enter the year for the target (year most be between 2015 and 2301): ')
comp_simu = input('Enter the name of the simulation you want to compare with the target (e.g LSCE_GRISLI2): ')
...
nb_exp = simulation.items()[simulation.items().index((comp_simu, simulation[comp_simu]))][1]  
...
for i, exp in enumerate(exp_list):
    ...
    comp_data = xr.open_dataset(f'{config.PATH_MASK}/{comp_simu}/grounding_mask_{comp_simu}_{exp}.nc')

    rmse_target_comp = ismip.compute_rmse(target_mask, comp_data)
    ...
    save_dir = f'{config.PATH_RMSE}/{target_simu}_{comp_simu}'
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f'{config.PATH_RMSE}/{target_simu}_{comp_simu}/RMSE_{target_simu}_{target_year}_{comp_simu}.png', dpi=300)
print('Figure saved successfully')

#min table
min_df = pd.DataFrame(min_table)
min_df.to_csv(f'{config.PATH_RMSE}/{target_simu}_{comp_simu}/min_RMSE_{target_simu}_{target_year}_{comp_simu}.csv', index=False)
print("Minimum table saved successfully")

#global summary CSV 
mini = pd.read_csv(f'{config.PATH_RMSE}/{target_simu}_{comp_simu}/min_RMSE_{target_simu}_{comp_simu}.csv')
...
summary_df.to_csv(f'{config.PATH_RMSE}/Summary_{target_simu}_{comp_simu}.csv', index=False)
